...
################################################## START BFS ALGORITHM ##################################################
    ...
    def bfs_traversal(self):
        ...
        while queue and not self.pause_var.get():
            ...
            self.status_label.config(text=f"Visiting node {node}, Path: {path}, Path Cost: {path_cost}", font=("Arial", 16, "bold", "italic"))
            self.master.update()
            ...
            if node == search_node:
                self.status_label.config(
                    text=f"Node {search_node} found! Path: {path}, Path Cost: {path_cost}, Enqueues: {self.enqueue_count}, Queue Size: {self.queue_size}", font=("Arial", 16, "bold", "italic"))
                self.bfs_running = False
        ...
        self.status_label.config(
            text=f"All nodes visited. Enqueues: {self.enqueue_count}, Queue Size: {self.queue_size}", font=("Arial", 16, "bold", "italic"))
        self.bfs_running = False
...
################################################## START DFS ALGORITHM ##################################################
    ...
    def dfs_traversal(self):
        ...
        if not self.target_found:
            self.status_label.config(
                text=f"All nodes are already visited and Node {search_node} is not found", font=("Arial", 16, "bold", "italic"))
        self.dfs_running = False
...
################################################## START HILL CLIMBING ALGORITHM ##################################################
    def hill_climbing(self):
        ...
        while current_node != search_node and not self.pause_var.get():
            ...
            self.status_label.config(text=f"Moving from {current_node} to {next_node}, Path: {path}, Total Cost: {total_cost}", font=("Arial", 16, "bold", "italic"))
            self.master.update()
            time.sleep(1 / self.speed_var.get())  # Adjust speed
        ...
        if current_node == search_node:
            self.status_label.config(
                text=f"Node {search_node} found! Path: {path}, Total Cost: {total_cost}", font=("Arial", 16, "bold", "italic"))
        else:
            self.status_label.config(
                text=f"Hill climbing stopped. Node {search_node} not found! Total cost: {total_cost}", font=("Arial", 16, "bold", "italic"))
    ...
    def beam_search(self):
        ...
        while beam and not self.pause_var.get():
            ...
            for current_node, total_cost, path in next_beam:
                self.update_node_color(current_node, 'green')  # Mark as visited
                self.status_label.config(text=f"Visiting node {current_node}, Path: {path}, Total Cost: {total_cost}", font=("Arial", 16, "bold", "italic"))
                self.master.update()
                time.sleep(1 / self.speed_var.get())  # Adjust speed
        ...
        self.status_label.config(
            text=f"Beam search stopped. Node {search_node} not found! Total Cost: {total_cost}", font=("Arial", 16, "bold", "italic"))
        self.beam_search_running = False
